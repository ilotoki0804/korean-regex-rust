...
impl Order {
    fn compile(&self) -> CompiledOrders {
        match self {
            Order::Default => {
                let chosungs: Vec<char> = "ㄱㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎ".chars().collect();
                let jungsungs: Vec<char> = "ㅏㅐㅑㅒㅓㅔㅕㅖㅗㅘㅙㅚㅛㅜㅝㅞㅟㅠㅡㅢㅣ".chars().collect();
                let jongsungs_with_zero: Vec<char> = "0ㄱㄲㄳㄴㄵㄶㄷㄹㄺㄻㄼㄽㄾㄿㅀㅁㅂㅄㅅㅆㅇㅈㅊㅋㅌㅍㅎ".chars().collect();
                (chosungs, jungsungs, jongsungs_with_zero)
            },
            Order::RegularFirst => {
                let chosungs: Vec<char> = "ㄱㄴㄷㄹㅁㅂㅅㅇㅈㅊㅋㅌㅍㅎㄲㄸㅃㅆㅉ".chars().collect();
                let jungsungs: Vec<char> = "ㅏㅑㅓㅕㅗㅛㅜㅠㅡㅣㅐㅒㅔㅖㅘㅙㅚㅝㅞㅟㅢ".chars().collect();
                let jongsungs_with_zero: Vec<char> = "0ㄱㄴㄷㄹㅁㅂㅅㅇㅈㅊㅋㅌㅍㅎㄲㄳㄵㄶㄺㄻㄼㄽㄾㄿㅀㅄㅆ".chars().collect();
                (chosungs, jungsungs, jongsungs_with_zero)
            }
        }
    }
...
mod test {
    ...
    fn test_compilestr() {
        ...
        assert_eq!("123[강당항은]", compilestr("123[ㄱㄷㅎ:ㅏ:ㅇ|은]", &orders).unwrap());
        assert_eq!("123[ㄱㄷㅎ:d:ㅇ|은]", compilestr("123[ㄱㄷㅎ:d:ㅇ|은]", &orders).unwrap());
